...
/**
 * Picker that includes a LongRangePicker for easier picking of dates in the past.
 * 
 * @author Jeff Dwyer
 *
 */
public class HDatePicker extends SimpleDatePicker implements DatePickerInterface {
	...
	protected void showCalendar(){
		...
		if(align == HorizontalPanel.ALIGN_RIGHT ){
			...
			//getAbsoluteLeft() is broken in FF for things in our gadgetDisplayer
			//seems to be a known problem, but no good fix. Even FireBug says that the 
			...
			Logger.log("SHOW RIGHT "+(this.getAbsoluteLeft() - WIDTH)+" "+(this.getAbsoluteTop() + this.getOffsetHeight()+4));
			
			
			//Logger.log("SHOW RIGHT "+getAbsoluteLeft(getElement())+" FIX "+getAbsoluteLeftFix(getElement()));
...
//    	while (elem) {
//      		left += elem.offsetLeft - elem.scrollLeft;
//      		parent = @com.google.gwt.user.client.DOM::getParent(Lcom/google/gwt/user/client/Element;)(elem);
//      		elem = elem.offsetParent;
//      		while (parent != elem) {
//        		left -= parent.scrollLeft;
//        		parent = @com.google.gwt.user.client.DOM::getParent(Lcom/google/gwt/user/client/Element;)(parent);
//      	 	}
//    	}
//    	return left + $doc.body.scrollLeft; 	  
//  }-*/;
