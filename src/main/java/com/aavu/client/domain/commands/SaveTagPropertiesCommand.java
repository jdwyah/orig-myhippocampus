...
public class SaveTagPropertiesCommand extends AbstractCommand implements Serializable {
		...
		for (int i = 0; i < metas.length; i++) {
			...
			if (meta.getTitle() != null && !meta.getTitle().equals("")) {
				...
				if (meta.getId() > 0) {
					...
					Meta saved = (Meta) CollectionUtils.getFromCollectionById(tagProperties
							.getMembers(), meta.getId());
