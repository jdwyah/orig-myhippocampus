...
public class BlogView extends Composite implements ExplorerPanel {
	...
	private void doLoadAll() {
		manager.getTopicCache().getAllTopicIdentifiers(loadAllStart, MAX_PER_PAGE,
				new StdAsyncCallback("Fetch Blog") {
	...
	/**
	 * Requires the sortedByDate map to have been created. It will poke the NavigationTool to upate
	...
	private void draw(int start) {
		...
		for (Iterator iterator = sortedByDate.getKeyList().iterator(); iterator.hasNext();) {
			...
			TopicPreviewLink previewLink = new TopicPreviewLink(fti, 100, null, previewPanel,
					manager);
