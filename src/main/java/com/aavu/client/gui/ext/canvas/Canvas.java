...
/**
 * Wrapper class for the Canvas HTML element It implements every method of the WhatWG specification.
 * 
...
public class Canvas extends FocusWidget implements SourcesMouseEvents {
	...
	/**
	 * Public constructor to create the Canvas element. The API was modified so it makes better use
	 * of the static-typedness of Java: instead of using getContext(String) the getContext2D method
	 * is provided to return a CanvasRenderingContext2D instance without the need for casting.
	 * 
	 * @param width
	 *            Canvas width
	 * @param height
	...
	public Canvas(int width, int height) {
		...
		if (getElement() == null)
			throw new RuntimeException("Canvas could not be created");
		...
		if (isVmlCanvasManagerPresent()) {
			Element element = DOM.createElement("<canvas width='" + width + "' height='" + height
					+ "'></canvas>");
			initElement(element);
		}
	 ...
	 * 
	 * @return the reference to the rendering context or null if the 2D rendering context is not
	 *         available.
	 */
	 ...
	 * specified format. The standard only requires the support of image/png, if the requested
	 * format is not supported then the implementation returns the data URL in PNG format. The
	 * caller should check whether the prefix of the returned URL equals to data:image/png or to the
	 * requested MIME type.
	 * 
	 * @param type
	 *            MIME type of the requested data URL, e.g. image/gif
	 * @return the String with the data URL
	 */
	 ...
	 * @return the JavaScriptObject of the requested Context, or null if the Context is not
	 *         available
	 */
