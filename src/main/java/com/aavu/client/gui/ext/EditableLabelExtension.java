...
/**
 * Editable Label class, funcionality displays a Label UI Element until clicked on, then if element
 * is set to be editable (default) then an editable area and Buttons are displayed instead.
 * 
 * If the Label is not set to be word wrapped (default) then the editable area is a Text Box and
 * clicking the OK button or hitting return key in the TextBox will display the Label with the
 * updated text.
 * 
 * If the Label is set to be word wrapped, using the setWordWrap(boolean) method, then the editable
 * area is a Text Area and clicking the OK button will display the Label with the updated text.
 * 
 * In both cases, clicking Cancel button or hitting Escape key in the TextBox/TextArea then the
 * Label is displayed with original text.
 * 
 * @author Adam Tacy
 * @version 0.0.2
 * 
 * Changes since version 0.0.1 + made cancelLabelChange public [ref request id: 1518134] + made
 * originalText have default value of empty string [to support ref request id: 1518134] *End*
 * 
 * HIPPO NOTE Only difference wrt to the original is that ours has a "hover" listener & associated
 * style change. ALSO CTOR will change "" string to "Change Me" so that there's someplace to click
 * 
 */
public class EditableLabelExtension extends Composite implements HasWordWrap, HasText {
	...
	/**
	 * String element that contains the original text of a Label prior to it being edited.
	 */
	 ...
	 * 
	 * @param flag
	 *            True or False value depending if the Label is to be editable or not
	 */
	...
	/**
	 * Returns the value of the isEditing flag, allowing outside users to see if the Label is being
	 * edited or not.
	 * 
	 * @return
	 */
	...
	/**
	 * Change the displayed label to be a TextBox and copy label text into the TextBox.
	 * 
	 */
	...
	/**
	 * Sets the Label text to the new value, restores the display and calls the update method.
	 * 
	 */
	...
	/**
	 * Creates the Label, the TextBox and Buttons. Also associates the update method provided in the
	 * constructor with this instance.
	 * 
	 * @param labelText
	 *            The value of the initial Label.
	 * @param onUpdate
	 *            The class that provides the update method called when the Label has been updated.
	 * @param visibleLength
	...
	/**
	 * Set the word wrapping on the label (if word wrapped then the editable field becomes a
	 * TextArea, if not then the editable field is a TextBox.
	 * 
	 * @param b
	 *            Boolean value, true means Label is word wrapped, false means it is not.
	 */
	...
	/**
	 * Constructor that changes default text for buttons and allows the setting of the wordwrap
	 * property directly.
	 * 
	 * @param labelText
	 ...
	 * @param cancelText
	 *            Text for use in overiding the default CANCEL button text.
	 * @param wordWrap
	 *            Boolean representing if the label should be word wrapped or not
	 */
	public EditableLabelExtension(String labelText, ChangeListener onUpdate, String okText,
	 ...
	 *            Boolean representing if the label should be word wrapped or not
	 */
	public EditableLabelExtension(String labelText, ChangeListener onUpdate, boolean wordWrap) {
	 ...
	 *            Handler object for performing actions once label is updated.
	 */
	public EditableLabelExtension(String labelText, ChangeListener onUpdate) {
		createEditableLabel(labelText, onUpdate, defaultOkButtonText, defaultCancelButtonText);
	}
}
