...
public class OverViewEther extends Ether {
	...
	protected int newSection(int depth, int key, Date date) {
		...
		if (lastYear != date.getYear() || lastMonth != date.getMonth()) {
			LabelWrapper startIntervalLabel = new LabelWrapper(overviewF.format(date), left,
					getIntervalTop() - 20);
			addObject(startIntervalLabel);
		}

		LabelWrapper startIntervalLabel = new LabelWrapper(TreeOfTime.getShortLabelForDepth(depth,
				key, date), left, getIntervalTop());
		addObject(startIntervalLabel);
