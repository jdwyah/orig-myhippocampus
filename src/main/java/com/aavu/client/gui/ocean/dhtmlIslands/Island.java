...
public class Island extends AbstractIsland implements ClickListener, SourcesMouseEvents,
		SourcesKeyboardEvents, RemembersPosition, DragEventListener {
	...
	public Island(TagInfo stat, OceanDHTMLImpl ocean, User user, Manager manager) {
		...
		// break;
		// case 2:
		// banner = new IslandBanner("The Republic of
		// "+tagStat.getTagName(),tagStat.getNumberOfTopics());
		// break;
		// case 3:
		...
		// int left = (int) (tagStat.getLongitude() * ocean.getLongitude()) -
		// gridToRelativeX(min_x);;
		...
		System.out.println(tagStat.getTagName() + " SIZE " + tagStat.getNumberOfTopics() + " minx "
				+ repr.min_x + " max x " + repr.max_x + " miny " + repr.min_y + " maxy "
				+ repr.max_y);
	...
	private void doPositioning(Widget minimumWidget) {
		...
		/*
		 * TODO clean this crud up. How do we size this DIV dynamically? Or take another look at
		 * putting these elements in another div.. but then we need to sort out drag-your-buddy
		 * system.
		 */
		...
		// System.out.println("width "+width+" Nx "+(repr.max_x + 1 - repr.min_x)+" Spacing
		// "+my_spacing+" img_size "+img_size);
		// System.out.println("height "+height+" Ny "+(repr.max_y + 1 - repr.min_y)+" Spacing
		// "+my_spacing+" img_size "+img_size);
		// }
	...
	private void situate(OceanDHTMLImpl o) {
		...
		if (tagStat.getLatitude() == 0 && tagStat.getLongitude() == 0) {
	...
	public void grow() {
		...
		clear();
		topicLabelList = null;// otherwise we'll find them and try to move them, but they've been
								// cleared
		levels.clear();
		add(banner, 0, 0);
		doIslandType(1);
	...
	public boolean possibleMoveOccurred() {
		...
		if (absLeft != left + oceanLeft || absTop != top + oceanTop) {
			int newLeft = absLeft - oceanLeft;
			int newTop = absTop - oceanTop;
			System.out.println("\n\n\n\nMove DETECTED!!!!!!!!!!!! " + tagStat.getTagName()
					+ " Scale " + scale + " newLeft " + newLeft + " newTop " + newTop + " "
					+ oceanLeft + " " + oceanTop);
			// ocean.islandMoved(tagStat.getTagId(), newLeft, newTop);
	...
	public boolean isWithin(int winLeft, int winTop, int winRight, int winBottom) {
		...
		return (left + scaledWidth > winLeft && left < winRight && top + scaledHeight > winTop && top < winBottom);
	}
	...
	public void showTopics() {
		if (!haveShownTopics) {
			...
			manager.getTopicCache().getTopicsWithTag(tagStat.getTagId(),
	...
	public void dragFinished(Widget dragging) {
		...
		// System.out.println("finished dragging "+label.getText()+" "+label.getXPct()+"
		// "+label.getYPct()+" "+xPct+" "+yPct);
	...
	private void topicMoved(DraggableTopicLabel label) {
		...
		manager.getTopicCache().saveTopicLocationA(tagStat.getTagId(), label.getTopicId(),
				(int) (label.getXPct() * width), (int) (label.getYPct() * height),
	...
	public void redraw(Topic t) {
		...
		System.out.println("redraw " + tagStat.getLatitude() + " " + tagStat.getLongitude()
				+ " to " + t.getLatitude() + " " + t.getLongitude());

		// NOTE. This makes some sense, but doesn't work, because the tag passed in is often just a
		// shell,
		// created from a TopicIdentifier in command.getAffectedTopics()
		// problem exposes itself as 0,0 lat/long when creating a new topic then adding it to an
		// island.
