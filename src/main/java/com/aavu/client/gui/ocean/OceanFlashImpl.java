...
public class OceanFlashImpl extends FlashContainer implements SpatialDisplay {
	...
	public void load(Topic topic, LoadFinishedListener listener) {
		...
		manager.getTopicCache().getTagStats(new StdAsyncCallback("Get Tag Stats") {
			public void onSuccess(Object result) {
				...
				// runCommandDeferred(getCommand(manager.getUser(),tagStats,
				// manager.getTopicCache().getNumberOfTopics()));
	...
	protected void callbackOverride(String command, int int0, final int int1, final int int2) {
		...
		if (command.equals("isleMovedTo")) {
			...
			// manager.getTopicCache().getTopicByIdA(int0, new
			// StdAsyncCallback("GetTopicById"){
			//
			// public void onSuccess(Object result) {
			// super.onSuccess(result);
			...
			// t.setLatitude(int2);
			// t.setLongitude(int1);
			// manager.getTopicCache().save(t, new
			// StdAsyncCallback("SaveLatLong"){});
			// }
			//				
			// });
		}
	...
	protected String islandObj(long id, String name, int size, int latitude, int longitude) {
		return "<object>" + numberPropChangeZeroAndNeg1ToNull("id", id) + stringProp("tag", name)
				+ numberPropChangeZeroAndNeg1ToNull("size", size)
				+ numberPropChangeZeroAndNeg1ToNull("xx", latitude)
				+ numberPropChangeZeroAndNeg1ToNull("yy", longitude) + "</object>";
	}
	...
	protected String getCommand(User user, TagStat[] tagStats, int totalNumberOfTopics) {
		...
		// sb.append("<invoke name=\"initLand\"
		...
		// sb.append("<property
		// id='1'>"+islandObj(8,"Contacts",8)+"</property>");
		// sb.append("<property
		// id='2'>"+islandObj(24,"Books",10)+"</property>");
		sb.append("</array>");
