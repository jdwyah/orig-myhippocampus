...
public class SearchServiceImpl implements SearchService, InitializingBean {
	...
	private List<SearchResult> search(final String searchString, final User user, final int start,
		...
		// nested exception is org.apache.lucene.queryParser.ParseException: Encountered "<EOF>" at
		// line 1, column 0.
		...
		// TODO how secure is this really? say somebodie's username is 'j*' will they get to see
		// 'jdwyah' stuff? eek.
		// Maybe do a check before we return results?
		//
		// should we be parsing that searchString? forums said '???' as a search string could make
		// things explode
		//
		CompassHitsOperations hits = compassTemplate.executeFind(new CompassCallback() {
			public Object doInCompass(CompassSession session) throws CompassException {
				...
				// "jack")).addMust(queryBuilder.lt("birthdate", "19500101"))
				// .toQuery().hits();
				...
				if (mustEqualMap != null) {
					...
					CompassQueryFilter queryFilter = queryFilterBuilder.query(booleanQueryBuilder
							.toQuery());
				...
				for (int i = 0; i < hits.length(); i++) {
					try {
						...
						hits.highlighter(i).fragment("text"); // this will cache the highlighted
						// fragment
		...
		for (int i = 0; i < hits.length(); i++) {
			...
			if (obj instanceof Entry) {
				...
				// res = new SearchResult(top.getId(), defaultCompassHit.getScore(), top
				// .getTitle(), null);
				// } else {
