 ...
 */
public class RemoteServiceServletReplacement extends HttpServlet {
   ...
   * 
   * This really a hack since we are assuming that every character will consume
   * two bytes upon transmission. This is definitely not true since some
   * characters actually consume more than two bytes and some consume less. This
   * is even less accurate if the string is converted to UTF8. However, it does
   * save us from converting every string that we plan on sending back to UTF8
   * just to determine that we should not compress it.
   */
   ...
   */
  public RemoteServiceServletReplacement() {
  ...
  public String processCall(String payload) throws SerializationException {
    ...
    ServerSerializationStreamWriterWithHibernateEscaping streamWriter = new ServerSerializationStreamWriterWithHibernateEscaping(
        serializableTypeOracle);
   ...
   * @return
   */
  private String createResponse(ServerSerializationStreamWriterWithHibernateEscaping stream,
   ...
   * 
   * NOTE we do not check that the type is serializable here. We assume that it
   * must be otherwise the application would never have been allowed to run.
   * 
  ...
  private boolean isImplementedRemoteServiceInterface(String intfName) {
    synchronized (knownImplementedInterfaces) {
      ...
      //
      while ((cls != null) && !RemoteServiceServletReplacement.class.equals(cls)) {
