...
import com.aavu.client.domain.mapper.generated.AbstractMapper;

public class MindTree extends AbstractMapper {
	 ...
	 * @param node
	 */
	private void populate(Set side,NavigableMindNode node){
		
		List stack = new ArrayList();
		List nodestack = new ArrayList();
		
		for (Iterator iter = side.iterator(); iter.hasNext();) {
			MindTreeElement element = (MindTreeElement) iter.next();
			...
			while(9==9){
				...
				System.out.println("el "+element.getLft()+" er "+element.getRgt()+" atl "+addTo.getLft()+" atr "+addTo.getRgt());
				if(element.getLft() <  addTo.getLft()
						||
						element.getRgt() > addTo.getRgt()){
					System.out.println("><");
	...
	private void setSide(Set side, NavigableMindNode node){
		...
		side.add(new MindTreeElement(node.getData(),null,curIdx,addChildren(side,curIdx,node)));
				
	}

	/**
	 * recursively create the Modified Preorder Tree Traversal
