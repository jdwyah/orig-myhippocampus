...
public class FilePersistanceServiceImplTest extends BaseTestWithTransaction {
	...
	public void testSaveFileStringIntByteArrayStringUser() throws HippoException {
		...
		String key = fileService.saveFile("text/plain", TOPIC_ID, orig_bytes, FILE_NAME, userService.getCurrentUser());
...
//		System.out.println(f.getAbsolutePath());
		...
		S3File s3 = null;
		try{
			PersistedFile fileToSave = new PersistedFile(IOUtils.getStreamAsByteArray(inputStream),"text/plain","build.xml");
			...
			assertTrue(s3.getUri().startsWith(userService.getCurrentUser().getUsername()+"/"+t.getId()+"/"));
